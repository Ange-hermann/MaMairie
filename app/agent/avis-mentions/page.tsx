...
export default function AvisMentionsAgentPage() {
  ...
  const filteredAvis = avis.filter(a => {
    ...
    if (activeTab === 'validation') {
      ...
      // Onglet Traitement : en_traitement, documents_verifies, approuvee, rejetee
      matchTab = a.statut === 'en_traitement' || a.statut === 'documents_verifies' || a.statut === 'approuvee' || a.statut === 'rejetee'
    }
  ...
  const stats = {
    ...
    en_cours_traitement: avis.filter(a => a.statut === 'en_traitement' || a.statut === 'documents_verifies').length,
    termines: avis.filter(a => a.statut === 'approuvee' || a.statut === 'rejetee').length,
  }
  ...
  const handleApprouver = async (avis: any) => {
    if (!confirm('Approuver et générer l\'acte avec mention ?')) {
      return
    }
    ...
    try {
      ...
      if (acteError || !acte) {
        throw new Error('Acte original non trouvé')
      }
      ...
      const { error: updateActeError } = await supabase
        .from(tableName)
        .update({ 
          a_mention: true,
      ...
      if (updateActeError) throw updateActeError
      ...
      alert('✅ Avis approuvé et mention ajoutée à l\'acte !\n\nL\'acte a été mis à jour avec la mention.')
      fetchData()
    } catch (error: any) {
